...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        // অ্যাপের অন্য কোথাও সেটিংস আপডেট হলে যেন লাইভ রিফ্রেশ হয়
    ...
    const localStats = React.useMemo(() => {
        ...
        records.forEach(f => {
            ...
            if (f.isMohollaMember) {
                mohollaMember.total++;
                if (isFemaleHead) mohollaMember.female++;
                else mohollaMember.male++;
            }

            if (f.isBloodDonorMember) {
                donerMember.total++;
                if (isFemaleHead) donerMember.female++;
                else donerMember.male++;
            }

            if (f.isTemporaryMember) {
                temporaryMember.total++;
                if (isFemaleHead) temporaryMember.female++;
                else temporaryMember.male++;
            }

            if (Array.isArray(f.members)) {
                f.members.forEach(m => {
                    const isFemale = m.gender === 'Female' || m.gender === 'মহিলা' || m.gender === 'নারী';
                    if (m.isMohollaMember) {
                        mohollaMember.total++;
                        if (isFemale) mohollaMember.female++;
                        else mohollaMember.male++;
                    }
                    if (m.isBloodDonorMember) {
                        donerMember.total++;
                        if (isFemale) donerMember.female++;
                        else donerMember.male++;
                    }
                    if (m.isTemporaryMember) {
                        temporaryMember.total++;
                        if (isFemale) temporaryMember.female++;
                        else temporaryMember.male++;
                    }
                });
            }
    ...
    // Counts by blood group from local records context
        ...
        <div className="main-section min-h-screen bg-[#F8FAFC] text-slate-800 font-sans selection:bg-[#1B8A44] selection:text-white">
            ...
            <section className="hero-section relative text-white pt-8 pb-16 px-4 overflow-hidden border-b-4 border-[#1B8A44]">
                ...
                <div className="max-w-6xl mx-auto relative z-10 text-center">
                    ...
                    {/* Dynamic Statistics Bar (API Driven with Male / Female breakdown) */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5 max-w-6xl mx-auto bg-white/10 backdrop-blur-md border border-white/15 p-4 rounded-2xl shadow-2xl">
                        {/* 1. নিবন্ধিত পরিবার */}
                        <div className="p-3 text-center border-r border-white/10 last:border-0 flex flex-col justify-between">
                            <div>
                                <div className="text-2xl sm:text-3xl md:text-4xl font-black text-emerald-300 font-mono">
                                    {displayTotalFamilies}
                                </div>
                                <div className="text-xs text-slate-200 font-medium mt-1 flex items-center justify-center gap-1">
                                    <Users size={13} className="text-emerald-400" /> নিবন্ধিত পরিবার
                                </div>
                            </div>
                        ...
                        {/* 2. মহল্লা সদস্য with Male / Female at bottom */}
                        <div className="p-3 text-center border-r border-white/10 last:border-0 flex flex-col justify-between">
                            <div>
                                <div className="text-2xl sm:text-3xl md:text-4xl font-black text-indigo-300 font-mono">
                                    {displayMohollaMembers.total}
                                </div>
                                <div className="text-xs text-slate-200 font-medium mt-1 flex items-center justify-center gap-1">
                                    <Building2 size={13} className="text-indigo-300" /> মহল্লা সদস্য
                                </div>
                            </div>
                            <div className="text-[10px] sm:text-[11px] text-indigo-100/90 font-medium mt-2 pt-1.5 border-t border-white/10 flex items-center justify-center gap-1.5 flex-wrap">
                                <span>Male: <strong className="text-white font-mono">{displayMohollaMembers.male}</strong> জন</span>
                                <span className="text-white/40">•</span>
                                <span>Female: <strong className="text-white font-mono">{displayMohollaMembers.female}</strong> জন</span>
                            </div>
                        </div>

                        {/* 3. অস্থায়ী সদস্য with Male / Female at bottom */}
                        <div className="p-3 text-center border-r border-white/10 last:border-0 flex flex-col justify-between">
                            <div>
                            ...
                            <div className="text-[10px] sm:text-[11px] text-amber-100/90 font-medium mt-2 pt-1.5 border-t border-white/10 flex items-center justify-center gap-1.5 flex-wrap">
                                <span>Male: <strong className="text-white font-mono">{displayTemporaryMembers.male}</strong> জন</span>
                                <span className="text-white/40">•</span>
                                <span>Female: <strong className="text-white font-mono">{displayTemporaryMembers.female}</strong> জন</span>
                            </div>
                        </div>

                        {/* 4. মোট রক্তদাতা with Male / Female at bottom */}
                        <div className="p-3 text-center border-r border-white/10 last:border-0 flex flex-col justify-between">
                            <div>
                            ...
                            <div className="text-[10px] sm:text-[11px] text-rose-100/90 font-medium mt-2 pt-1.5 border-t border-white/10 flex items-center justify-center gap-1.5 flex-wrap">
                                <span>Male: <strong className="text-white font-mono">{displayDonerMembers.male}</strong> জন</span>
                                <span className="text-white/40">•</span>
                                <span>Female: <strong className="text-white font-mono">{displayDonerMembers.female}</strong> জন</span>
                            </div>
                        </div>
            ...
            {/* 4. BLOOD DONATION BENEFITS SECTION */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="bg-white border-2 border-slate-200 hover:border-rose-400 p-6 rounded-2xl shadow-sm hover:shadow-xl transition duration-300 group">
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                            নিয়মিত রক্তদানে রক্তে অতিরিক্ত আয়রন জমতে পারে না। ফলে রক্তনালী পরিষ্কার থাকে এবং হার্ট অ্যাটাক ও স্ট্রোকের ঝুঁকি বহুগুণ হ্রাস পায়।
                        </p>
                    </div>
                    ...
                    <div className="bg-white border-2 border-slate-200 hover:border-emerald-400 p-6 rounded-2xl shadow-sm hover:shadow-xl transition duration-300 group">
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                            রক্তদানের ২৪ থেকে ৪৮ ঘণ্টার মধ্যে শরীরে নতুন রক্তকণিকা ও প্লাজমা তৈরি হওয়া শুরু হয়, যা শরীরের রোগ প্রতিরোধ ক্ষমতা বৃদ্ধিতে সহায়ক।
                        </p>
                    </div>
                    ...
                    <div className="bg-white border-2 border-slate-200 hover:border-blue-400 p-6 rounded-2xl shadow-sm hover:shadow-xl transition duration-300 group">
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                            রক্তদানের পূর্বে হিমোগ্লোবিন, রক্তচাপ, হেপাটাইটিস-বি ও সি, সিফিলিস ও এইচআইভি সহ ৫টি বড় রোগের স্বাস্থ্য পরীক্ষা সম্পূর্ণ বিনামূল্যে পাওয়া যায়।
                        </p>
                    </div>
                    ...
                    <div className="bg-white border-2 border-slate-200 hover:border-amber-400 p-6 rounded-2xl shadow-sm hover:shadow-xl transition duration-300 group">
                        ...
                        <h3 className="text-lg font-bold text-[#0F2C59] font-serif mb-2">
                            ৪. ক্যালোরি ক্ষয় ও ওজন নিয়ন্ত্রণ (Weight Management)
                        </h3>
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                            একবার রক্তদান করলে শরীরে প্রায় ৬৫০ ক্যালোরি বার্ন হয়, যা অতিরিক্ত চর্বি কমায় এবং ওজন নিয়ন্ত্রণে ভারসাম্য রক্ষা করে।
                        </p>
                    </div>
                    ...
                    <div className="bg-white border-2 border-slate-200 hover:border-[#1B8A44] p-6 rounded-2xl shadow-sm hover:shadow-xl transition duration-300 group">
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                            একজন মুমূর্ষু রোগীর জীবন বাঁচাতে রক্তদানের মাধ্যমে যে আত্মতৃপ্তি ও পরম মানসিক শান্তি পাওয়া যায়, তা অসামান্য মানবিক ও দ্বীনি অনুভূতি।
                        </p>
                    </div>
                    ...
                    <div className="bg-white border-2 border-slate-200 hover:border-indigo-400 p-6 rounded-2xl shadow-sm hover:shadow-xl transition duration-300 group">
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                            শরীরে আয়রনের পরিমাণ সুষম থাকায় লিভার, ফুসফুস ও পাকস্থলীর ক্ষতিকর ক্যানসার কোষ সৃষ্টির সম্ভাবনা উল্লেখযোগ্য হারে হ্রাস পায়।
                        </p>
                    </div>
                </div>
            </section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl text-center space-y-3">
                            ...
                            <p className="text-xs text-slate-600 leading-relaxed">
                                পটিয়া ৯নং ওয়ার্ডের স্থায়ী ও অস্থায়ী বাসিন্দাদের পরিবার প্রধান, স্ত্রী, পুত্র ও কন্যাদের সঠিক ডিজিটাল রেকর্ড ডাটাবেজ।
                            </p>
                        </div>
                        ...
                        <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl text-center space-y-3">
                            ...
                            <p className="text-xs text-slate-600 leading-relaxed">
                                ওয়ার্ডের বাইরে বসবাসকারী বা প্রবাসী শুভাকাঙ্ক্ষীদের জন্য বিশেষ &quot;External Member&quot; ট্যাগ সহ ডাইরেক্টরি অন্তর্ভুক্তি।
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="cta-section bg-gradient-to-r from-[#0F2C59] via-[#153D7A] to-[#0A1D3B] text-white rounded-3xl p-8 sm:p-10 shadow-2xl border-2 border-[#1B8A44] flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="space-y-2 text-center md:text-left">
                        ...
                        <p className="text-xs sm:text-sm text-slate-300 max-w-xl">
                            যে কোনো সময়ে রক্তের প্রয়োজন হলে সরাসরি আমাদের সদস্য ডাইরেক্টিতে সার্চ করুন অথবা হটলাইনে যোগাযোগ করুন।
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                        ...
                        <a
                            href={`tel:${hotline.replace(/[০-৯]/g, d => '0123456789'['০১২৩৪৫৬৭৮৯'.indexOf(d)]).replace(/[^0-9+]/g, '')}`}

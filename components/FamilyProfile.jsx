...
export const FamilyProfile = ({
    ...
    const headAge = calculateExactAge(record.dob || record.age);
                ...
                <div className="bg-white border-2 border-[#0F2C59]/30 p-6 rounded-2xl shadow-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                        ...
                                        {record.isMohollaMember && (
                                            <span className="px-1.5 py-0.5 bg-emerald-500 text-white text-[9px] font-mono font-bold rounded uppercase">
                                                মহল্লা সদস্য
                                            </span>
                                        )}
                                        {record.isBloodDonorMember && (
                                            <span className="px-1.5 py-0.5 bg-rose-500 text-white text-[9px] font-mono font-bold rounded uppercase">
                                                রক্ত দাতা সদস্য
                                            </span>
                                        )}
                                        {record.isTemporaryMember && (
                                            <span className="px-1.5 py-0.5 bg-purple-500 text-white text-[9px] font-mono font-bold rounded uppercase">
                                    ...
                                    {headAge && (
                                        <div className="mt-1.5">
                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-900 border border-amber-300 rounded text-xs font-bold shadow-2xs">
                        ...
                        {record.members.map((mem, idx) => {
                            const memberAge = calculateExactAge(mem.dobOrAge || mem.dob || mem.age);
                            return (
                                <div key={mem.id || idx} className="bg-slate-50 border border-slate-300 p-4 rounded-xl shadow-xs">
                                        ...
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-bold text-slate-900 flex items-center gap-2 flex-wrap">
                                                    <span>{mem.name || 'সদস্য'}</span>
                                                    {memberAge && (
                                                        <span className="text-[11px] px-2 py-0.5 bg-amber-100/90 text-amber-900 border border-amber-300 rounded font-bold">
                                                            {memberAge}
                                                        </span>
                                                    )}
                                                </h4>
                                    ...
                                    <div className="mt-3 space-y-1 text-xs text-slate-700">
                                        <div className="flex justify-between items-center">
                                            <span className="text-slate-500">লিঙ্গ ও জন্ম/বয়স:</span>
                                            <span className="font-medium">{mem.gender} • {mem.dobOrAge || '—'}</span>
                                        </div>
                                        {memberAge && (
                                            <div className="flex justify-between items-center bg-amber-50 px-2 py-1 rounded border border-amber-200 text-amber-950 font-medium">
                                                <span className="text-amber-900 font-semibold">হিসাবকৃত বয়স:</span>
                                                <span className="font-bold">{memberAge}</span>
                                            </div>
                                        )}
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        {record.isMohollaMember && (
                                            <span className="px-2.5 py-0.5 bg-emerald-500 text-white border border-emerald-300 rounded-md text-xs font-mono font-extrabold tracking-wider uppercase shadow-2xs">
                                                মহল্লা সদস্য
                                            </span>
                                        )}
                                        {record.isBloodDonorMember && (
                                            <span className="px-2.5 py-0.5 bg-rose-500 text-white border border-rose-300 rounded-md text-xs font-mono font-extrabold tracking-wider uppercase shadow-2xs">
                                                রক্ত দাতা সদস্য
                                            </span>
                                        )}
                                        {record.isTemporaryMember && (
                                            <span className="px-2.5 py-0.5 bg-purple-500 text-white border border-purple-300 rounded-md text-xs font-mono font-extrabold tracking-wider uppercase shadow-2xs">
                                                ভাড়াটিয়া/অস্থায়ী সদস্য
                                            </span>
                                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200">
                            <div className="space-y-3">
                                ...
                                <div className="space-y-2 text-xs">
                                    ...
                                    {headAge && (
                                        <div className="flex justify-between border-b border-slate-100 pb-1 bg-amber-50 px-2 py-0.5 rounded text-amber-950">
                                            <span className="text-amber-900 font-semibold">বর্তমান বয়স:</span>
                                            <span className="font-bold">{headAge}</span>
                                        </div>
                                    )}
                                    <div className="flex justify-between border-b border-slate-100 pb-1">

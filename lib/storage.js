...
export function searchRecords(records, options = {}, argBloodGroup = '', argStatus = '') {
  ...
  return records.filter(rec => {
    ...
    if (bloodF) {
      ...
      const memberBloodMatch = rec.members && rec.members.some(m => m.bloodGroup === bloodF);
    ...
    if (nameQ) {
      ...
      const membersMatch = rec.members && rec.members.some(m => matchNameInString(m.name));
...
export function getAppSettings() {
  ...
  try {
    ...
    return data ? JSON.parse(data) : { logo: '' };
  } catch (e) {
    return { logo: '' };
  }
}
